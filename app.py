...
def forget_password():
    if request.method == 'POST':
        ...
        return redirect(url_for('#' if user_type == 'patient' else 'doctor_login'))
...
def doctor_reset_password(token):
    ...
    if request.method == 'POST':
        ...
        flash("Password reset successful! You can now log in.", "success")
        return redirect(url_for('docto_login'))
...
def doctor_signup():
    ...
    return render_template('doctor__signup.html')
...
def upload_plan(appointment_id):  # Change 'patient_id' to 'appointment_id'
    ...
    try:
        ...
        # ✅ Insert Therapy Plan using Correct `patient_id`
        cur.execute("""
            INSERT INTO therapy_plans (patient_id, doctor_id, file_path, upload_date)
            VALUES (%s, %s, %s, NOW())
        """, (patient_id, session['doctor_id'], file_path))
        mysql.connection.commit()

        # ✅ Update therapy_plan status
        cur.execute("""
            UPDATE appointment_requests 
            SET therapy_plan_status = 'Uploaded' 
            WHERE id = %s
        """, (appointment_id,))
        mysql.connection.commit()
